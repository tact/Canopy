...
public struct CKSubscriptionError: CKTransactionError, Codable, Equatable {
  ...
  public init(from error: Error) {
    ...
    if let ckError = error as? CKError {
      ...
      batchErrors = CKSubscriptionError.parseBatchErrors(dict: ckError.partialErrorsByItemID ?? [:])
    } else {
      ...
      batchErrors = CKSubscriptionError.parseBatchErrors(dict: (nsError.userInfo[CKPartialErrorsByItemIDKey] as? [AnyHashable: Error]) ?? [:])
    }
  }
